...
step from a given polynomial $r_{n}$ a polynomial $r_{i_{n}}$ which
was used to generate $r_{n}$ as $Spoly(r_{i_{n}},r_{j_{n}})$. The
resulting S-pair-chain has the form $\{r_{q},\ldots,r_{i_{i_{l}}},r_{i_{l}},r_{l}\}$
...
1 so there is an S-pair-chain $\{r_{1},\ldots,r_{i_{i_{l}}},r_{i_{l}},r_{l}\}$.

Now assume that some labeled polynomial $r_{n}$ is known to be a
...
of length 2 chains coming from $r_{n}$. So, if labeled polynomial
$r_{n}$ is chain-generator, we can select another chain generator
from it's S-pair-descendants.

Using theses result we can find infinite S-pair-chain starting with
$r_{1}$ and consisting of chain generators which proves the following
\begin{thm}
...
are strictly descending according to term ordering. This fact can't
be used directly to show chains finiteness because unlike the ordering
of monomials the ordering of monomial quotients is not well ordering
-- for example the sequence $\frac{x}{x}>\frac{x}{x^{2}}>\cdots>\frac{x}{x^{n}}>\cdots$
...
\begin{itemize}
\item $y\cdot b_{1}\gtrdot100y\cdot b_{2}$ because signature of left side
is $\succ$
\item $x\cdot b_{1}\gtrdot y\cdot b_{1}$ because signature of left side
is $\succ$
\item $-x\cdot b_{1}$ and $2x\cdot b_{1}$ are not comparable because signatures
and list indices are equal
\item $y^{2}\cdot b_{1}\lessdot y\cdot b_{3}$ because signature of left
side is $\prec$
\item $x^{2}\cdot b_{1}\gtrdot x\cdot b_{3}$ because signatures are equal
and the list position of left side's labeled polynomial is 1 which
is smaller than right side's position 3.
\end{itemize}
