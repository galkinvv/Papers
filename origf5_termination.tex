...
of labeled polynomials $\left\{ r_{1},r_{2},\ldots,r_{m},\ldots,r_{n},\ldots\right\} $
...
that sequence with the property that $r_{k_{n+1}}$ is an S-polynomial
...
form $\{r_{q},\ldots,r_{i_{i_{l}}},r_{i_{l}},r_{l}\}$ where all polynomials
...
$\Sig(r_{l_{0}})=s$ and an ordered by generating time subset $\{r_{l_{1}},\ldots,r_{l_{n}},\ldots\}$
...
$\Sig(h_{i})|\Sig(h_{i+1})|\cdots|\Sig(h_{j})$ and $\frac{\HM(h_{i})}{\Sig(h_{i})}>\frac{\HM(h_{i+1})}{\Sig(h_{i+1})}>\cdots>\frac{\HM(h_{j})}{\Sig(h_{j})}$.
...
as an ordered integer sequence $G_{g}=\{b_{1},\ldots,b_{N}\}$ with
$b_{j}<b_{j+1}$. It should be noted that this order does correspond
to the order of labeled polynomials in the sequence produced by concatenated
rule arrays $Rule[m]:Rule[m-1]:\cdots:Rule[1]$ because addition of
new polynomial to $R$ is always followed by addition of corresponding
...
of infinite $\gtrdot$-descending sequence of representation elements
...
with ordering $\lessdot$.

The straightforward proof for well-orderness of representations with
$\lessdot$ following from well-orderness of elements with $\lessdot$
is not very complex but to skip its strict details the theorem 2.5.5
of \cite{Multiset-Order} will be used. It states well-orderness of
finite multiset with an lexicographically extended ordering of well-ordered
elements. This applies to the representations because they form a
subset in the finite multiset of representation elements. So, we have
\begin{thm}
The representations are well-ordered with $\lessdot$ ordering.
\end{thm}
...
\end{thm}
In conjunction with representation well-orderness this means that
...
by IsReducible is empty set. It means that one of conditions (a) -
(d) was not satisfied for all polynomials in $G\cup Done$ including
$f$. This is not possible because:
\begin{itemize}
