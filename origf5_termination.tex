...
The Faugère's F$_{5}$ algorithm is known to be efficient method for
Gröbner basis computation but one of the main problems with it's practical
...
inside AlgorithmF5 is infinite then the $d$ value infinitely grow.
\end{claim}
Let's suppose that there is an input $\left\{ f_{1},...,f_{m}\right\} $
over $\mathcal{K}[x_{1}...,x_{n}]$ for which original F$_{5}$ does
not terminate, and that it is shortest input of such kind -- the algorithm
do terminate on shorter input $\left\{ f_{2},...,f_{m}\right\} $.
...
during execution of the cycle inside AlgorithmF5. Let's call $d_{i}$
the value of $d$ on $i$-th cycle iteration and extend it to $d_{0}=-1$.
The simple property of $d_{i}$ is it's non-strict growth: $d_{i}\geqslant d_{i-1}$.
It holds because on the $i-1$-th iteration all polynomials in $R_{d}$
have degree $d_{i-1}$ and therefore all new generated critical pairs
have degree at least $d_{i-1}$. Now suppose that $j$ is number of
some iteration. At the iteration $j$ all critical pairs with degree
$d_{j}$ are extracted from $P$. After call to Reduction some new
critical pairs are added to $P$. There exist a possibility that some
of them has degree $d_{j}$. We're going to show that all such critical
pairs do not generate S-polynomials in the algorithm because they
are discarded .
...
that Rewritten?($u_{3-k},r_{3-k})$ returns true at the moment of
TopReduction execution, so it still returns true for all algorithm
execution after this moment because rewritings do not disappear.
...
are equal. This leads to fact that Rewritten?($u_{3-k},r_{3-k})$
returns true after adding rule corresponding to $r_{k}$ because $u_{3-k}\cdot r_{3-k}$
is rewritable by $1\cdot r_{k}$. So like in case (c) Rewritten?($u_{3-k},r_{3-k})$
...
$r_{k}$ in algorithm flow so for both cases (c) and (d) call to Rewritten?($u_{3-k},r_{3-k})$
...
AlgorithmF5 does terminate, in particular all calls to Reduction terminate.
\end{claim}
We need to get a simple fact about signatures of polynomials in $ToDo$
set inside Reduction algorithm. All the critical pairs created with
CritPair inside the AlgorithmF5 called for new input sequence item
$f_{i}$ have greater S-pair part with signature index $i$. So, all
polynomial initially forming $ToDo$ are generated by Spol and have
signature index $i$. The polynomials added in $ToDo$ inside Reduction
are generated by TopReduction. Each such polynomial has signature
$u\Sig(r_{k_{1}})$ that is greater than signature $\Sig(r_{k_{0}})$
...
the other hand all polynomials inside $ToDo$ has the same total signature
...
reduces the head monomial and the monomial order is well-founded.
Together with finiteness of $S_{0}$-signature polynomials count this
shows that the number of such steps is finite too.
...
Claims \ref{iterations-d_grow} and \ref{Every_cycle_iteration_finish}
together give the following result:
\begin{claim}
\label{d-does-grow}If the algorithm does not terminate for some input
then the value of $d$ infinitely grow during iterations.
\end{claim}
...
sequence infinitely grow. So, in this case there is an infinite sequence
of labeled polynomials $\left\{ r_{1},r_{2},...,r_{m},...,r_{n},...\right\} $
where $r_{1},...,r_{m}$ correspond to $m$ input polynomials and
...
existing polynomials $r_{i_{l}}$ and $r_{j_{l}}$ as $poly(r_{l})=u_{l}poly(r_{i_{l}})-v_{l}poly(r_{j_{l}})$
and $\Sig(r_{l})=u_{l}\Sig(r_{i_{l}})\succ v_{l}\Sig(r_{j_{l}})$.
...
$\left\{ r_{k_{1}},r_{k_{2}},...,r_{k_{n}},...\right\} $ in that
sequence with the property that $r_{k_{n+1}}$ is an S-polynomial
generated by $r_{k_{n}}=r_{i_{k_{n+1}}}$ and some other polynomial
$r_{j_{k_{n+1}}}$, where $r_{k_{n}}$ correspond to greater by signature
S-pair part , so $\Sig(r_{k_{n+1}})=u_{k_{n+1}}\Sig(r_{k_{n}})$ and
\begin{equation}
\Sig(r_{k_{n}})|\Sig(r_{k_{n+1}}).\label{eq:s-pair-chain-def}
\end{equation}
Finite or infinite labeled polynomial sequence which neighbor elements
satisfy property \ref{eq:s-pair-chain-def} will be called \emph{S-pair-chain}.
Every generated labeled polynomial $r_{l}$ has an S-pair-chain corresponding
to it. This chain can be constructed in reverse direction going from
it's last element $r_{l}$ by selecting every step from a given polynomial
$r_{n}$ a polynomial $r_{i_{n}}$ which was used to generate $r_{n}$
as $Spoly(r_{i_{n}},r_{j_{n}})$. The resulting S-pair-chain has the
form $\{r_{q},...,r_{i_{i_{l}}},r_{i_{l}},r_{l}\}$ where all polynomials
has the same signature index $q=index(r_{l})$ and the first element
is the input polynomial of that index.
...
in finite number of S-pair-chains of length 2.
\end{thm}
...
case the SPol itself is checking that $(u_{l};r_{i_{l}})$ is not
rewritten, in the TopReduction the check is in the IsReducible procedure.
And in both cases the computed S-polynomial is immediately added to
the list of rules, as the newest element. So, at the moment of the
construction of S-polynomial with signature $s$ we can assert that
the higher part of S-pair correspond to the newest rule with signature
dividing $s$ -- this part even may be determined by list of rules
and $s$ without knowing anything other about computation.

Consider arbitrary labeled polynomial $r_{l_{0}}$ with signature
$\Sig(r_{l_{0}})=s$ and an ordered by generating time subset $\{r_{l_{1}},...,r_{l_{n}},...\}$
of labeled polynomials with signatures of form $\Sig(r_{l_{n}})=\Sig(r_{l_{0}})\cdot u_{n}$.
From the signature divisibility point of view all of the possibly
infinite number of pairs $\{r_{l_{0}},r_{l_{n}}\}$ can be S-pair-chains
of length 2. But the ideal $\left(u_{i}\right)$ in $T$ is finitely
generated by Dickson's lemma, so after some step $i_{0}$ we have
$\forall i>i_{0}\,\exists j_{i}\leqslant i_{0}<i$ such that $u_{j_{i}}|u_{i}$.
So $\forall i>i_{0}$ the sequence $\{r_{l_{0}},r_{l_{i}}\}$ is not
S-pair-chain because $\Sig(r_{l_{0}})\cdot u_{i}$ is rewritten by
$\Sig(r_{l_{j_{i}}})\cdot\frac{u_{i}}{u_{j_{i}}}$ and no more than
$i_{0}$ S-pair-chains of length 2 with first element $r_{l_{0}}$
exist. This finishes the theorem proof. The set of ends of those 2-length
S-pair-chains will be called S-pair-descendants of $r_{l_{0}}$.

Proof of existence of infinite S-pair-chain inside infinite labeled
polynomial sequence require some caution while dealing with infinities,
...
with $r_{l}$.
\end{defn}
First we need to prove that input labeled polynomial $r_{1}=(f_{1},1F_{1})$
is chain generator if the algorithm doesn't terminate. Every labeled
polynomial $r_{l}$ generated in the last non-terminating call to
Algorithm F5 has signature index 1, so there exists an S-pair-chain
corresponding to $r_{l}$ which starts with $r_{1}$ and ends with
$r_{l}$. So there is infinite different chains starting from $r_{1}$
corresponding to labeled polynomials $r_{l}$, so $r_{1}$ is a chain
generator. Now assume that some labeled polynomial $r_{n}$ is known
to be a chain generator. Then one of the finite number of S-pair-descendants
of $r_{n}$ need to be a chain generator too, because in the other
case the number of different chains of length greater than 2 coming
from $r_{n}$ was limited by a finite sum of the finite counts of
chains coming from every S-pair-descendant, and the finite number
of length 2 chains coming from $r_{n}$.

So, if labeled polynomial $p$ is chain-generator, we can select another
chain generator from it's S-pair-descendants. Using this result we
can find infinite S-pair-chain starting with $r_{1}$ and consisting
of chain generators which proves the following
\begin{thm}
If the algorithm does not terminate for some input then there exists
infinite S-pair-chain $\{h_{i}\}$.
\end{thm}
For the next S-pair-chain property it is important that the polynomial
...
in the S-pair-chain retains the same $poly(h_{n})$ value that was
used for S-pair generation and we can state that
\[
...
\end{equation}
The monomial ordering can be transitively extended to monomial quotients:
$\frac{m_{1}}{m_{2}}>\frac{m_{3}}{m_{4}}\Leftrightarrow m_{1}m_{4}>m_{3}m_{2}$.
From the first inequality in \ref{eq:spair-chain} we can get $\frac{\HM(h_{n})}{\Sig(h_{n})}>\frac{\HM(h_{n+1})}{\Sig(h_{n+1})}$,
so in the S-pair-chain the quotients $\frac{\HM(h_{i})}{\Sig(h_{i})}$
are strictly descending according to term ordering.
...
result we have $i<j$ and with the S-pair chain properties we get
$\Sig(h_{i})|\Sig(h_{i+1})|\cdots|\Sig(h_{j})$ and $\frac{\HM(h_{i})}{\Sig(h_{i})}>\frac{\HM(h_{i+1})}{\Sig(h_{i+1})}>\dots>\frac{\HM(h_{j})}{\Sig(h_{j})}$.
...
labeled polynomials $f,g$ with $g$ generated after $f$ that satisfies
...
\[
\frac{\HM(f)}{\Sig(f)}>\frac{\HM(g)}{\Sig(g)},
\]
where the \textless{} corresponds to the term ordering.
\end{thm}
The proof of theorem\ref{thm:f_g_3_props} does not rely on any correspondence
between orderings on signatures and terms. But the original algorithm
F$_{5}$ uses the same ordering for both cases and now we utilize
this fact and make a transition from one to other to get relation
on signatures $\Sig(g)\succ t\cdot\Sig(f),\mbox{ where }t=\frac{\HM(g)}{\HM(f)}\in T$.
The last inequality shows that $tf$ can be used as a reducer for
$g$ in TopReduction from the signature point of view -- i.e. it satisfy
checks (a) and (d) inside IsReducible and it's signature is smaller.
In the absence of criteria this would directly lead to contradiction
...
$g$ by $f$. So we get the following result about termination:
\begin{fact}
If no polynomials are rejected by criteria checks (b) and (c) inside
IsReducible the algorithm does terminate.
\end{fact}
...
contradiction, so for the following parts of paper $g$ is treated
as some fixed labeled polynomial with signature index 1. If the algorithm
does not terminate then $g$ is assumed additionally to be a polynomial
from theorem \ref{thm:f_g_3_props}. Most of the following theory
is formulated for the objects with signatures $\prec\Sig(g)$.


\section{S-pairs with signatures smaller than $\Sig(g)$}

To prove the idea let us work with algorithm state just before adding
$g$ to $Done$ during call to AlgorithmF5 with $i=1$. Consider a
finite set $G_{1}\cup Done$ of labeled polynomials at that moment.
This set contains positions of labeled polynomials in $R$, so it's
elements can be ordered according to position in $R$ and written
as an ordered integer sequence $G_{g}=\{b_{1},...,b_{N}\}$ with $b_{j}<b_{j+1}$.
...
In this terminology we can say that all input polynomials $\left\{ f_{1},...,f_{m}\right\} $
...
by polynomials $r_{i_{1}}$ and $r_{j_{1}}$ such that $\HM(r_{j_{1}})|\HM(r_{i_{1}})$
so that S-polynomial has a form $poly(r_{i_{1}})-v_{1}poly(r_{j_{1}}).$
Such situation is possible for two $G_{g}$ elements if reduction
of $r_{i_{1}}$ by $r_{j_{1}}$ was forbidden in TopReduction or IsReducible.
So for this case the path of S-pair ``processing'' is the following:
\begin{itemize}
\item The S-pair part $v_{1}r_{j_{1}}$ is checked in IsReducible. (a) is
satisfied because $\HM(r_{j_{1}})|\HM(r_{i_{1}})$ . It can be rejected
by one of the other checks:

\begin{itemize}
\item Rejection by check (b) correspond to the normal form check $\varphi$
for $v_{1}r_{j_{1}}$
\item Rejection by check (c) correspond to the Rewritten? check for $v_{1}r_{j_{1}}$
\item Rejection by check (d) means that either $v_{1}r_{j_{1}}$ or $1r_{i_{1}}$
can be rewritten by other, so if S-pair was not rejected by check
(c) this type of rejection means that S-pair part $1r_{i_{1}}$ fails
to pass Rewritten? check.
\end{itemize}
\item The non-rejected in IsReducible S-pair is returned in the TopReduction.
Signature comparison in TopReduction forbids the reduction of $r_{i_{1}}$
by $r_{j_{1}}$ and returns computed S-polynomial corresponding to
...
generator polynomial $r_{i_{1}}$ such that $\totaldeg(r_{i_{1}})=\totaldeg(g),\,\Sig(r_{i_{1}})\prec\Sig(g)$.
...
from $ToDo$ in the signature increasing order, so $r_{i_{1}}$ is
reduced before $g$ and during $r_{i_{1}}$ reduction just before
putting $r_{i_{1}}$ to $Done$ a call to IsReducible starts processing
for all such S-pairs.

This finishes proof of the following result mentioning properties
which are very similar to the properties used in Theorem 21 in \cite{F5C}:
\begin{thm}
...
will be referenced as \emph{S-pairs with a computed $G_{g}$-representation}.
\end{enumerate}
...
Representations are used to order all possible ways how a labeled
...
algorithm are elements of ideal $\left(f_{1},...,f_{m}\right)$. So
any labeled polynomial $p$ can be written as $ $$\sum_{i}f_{i}g_{i}$,
...
\[
p=\sum_{k}m_{k}\cdot b_{i_{k}},\, m_{k}\in\mathcal{K}\times T,\, b_{i_{k}}\in\left\{ f_{1},...,f_{m}\right\} \subset G_{g}.
\]
\end{example}
\begin{defn}
...
\end{defn}
The definition of signature combined with this input-representation
gives the following result:
\begin{claim}
A labeled polynomial $p$ with known signature $\Sig(p)$ has an input-representation
consisting of an element $c\Sig(p)\cdot f_{index(p)}$ and some other
elements with smaller signatures.
\end{claim}
...
signature because it is possible to have $m_{1}\Sig(b_{i_{1}})=m_{2}\Sig(b_{i_{2}})$
while $i_{1}\ne i_{2}$.
\begin{example}
\label{example-of-having-gg-repr}The last example comes from S-pairs
with a computed $G_{g}$-representation. S-polynomial of $b_{i_{l}}$
and $b_{j_{l}}$ from $G_{g}$ is $p=u_{l}poly(b_{i_{l}})-v_{l}poly(b_{j_{l}})$.
...
$i_{l}$ and $j_{l}$ because corresponding labeled polynomial $b_{l}$
is added to $R$ at the moment of S-polynomial computation in Spol
or TopReduction so the polynomials $b_{i_{l}}$ and $b_{j_{l}}$ used
...
elements ordering}: $c_{i}t_{i}\cdot b_{i}\gtrdot c_{j}t_{j}\cdot b_{j}$
...
are comparable with $\lessdot$ order because they have distinct $\left(t_{k},b_{k}\right)$
by definition. Below are given some examples of $\lessdot$ element
ordering for the 3-element list $G_{g}=\left\{ b_{1},\, b_{2},\, b_{3}\right\} $
...
\[
y\cdot b_{1}\gtrdot100y\cdot b_{2},\,\mbox{because signature of left side is}\succ
\]
\[
x\cdot b_{1}\gtrdot y\cdot b_{1},\,\mbox{because signature of left side is}\succ
\]
\[
-x\cdot b_{1}\mbox{ and }2x\cdot b_{1}\,\mbox{are not comparable becuase signatures and list indices are equal}
\]
\[
y^{2}\cdot b_{1}\lessdot y\cdot b_{3},\,\mbox{because signature of left side is}\prec
\]
\[
x^{2}\cdot b_{1}\gtrdot x\cdot b_{3},
\]
because signatures are equal and the list position of left side's
labeled polynomial is 1 which is smaller than right's position 3.
...
a list with $\gtrdot$-decreasing order. This form can be used for
equality testing because if two representations are equal then they
have exactly equal ordered forms.
\begin{defn}
With ordered forms the \emph{$G_{g}$-representations ordering} can
be introduced: the representation $\sum_{k}m_{i_{k}}\cdot b_{i_{k}}$
is $\lessdot$-smaller than $\sum_{k}m_{j_{k}}\cdot b_{j_{k}}$ iff
the ordered form of the first representation is smaller than second's
according to lexicographical extension of $\lessdot$ ordering on
elements. For the corner case of the one ordered form being beginning
...
\[
x^{2}\cdot b_{1}+xy\cdot b_{1}+y^{2}b_{1}\gtrdot x^{2}\cdot b_{1}+100y^{2}\cdot b_{1},\mbox{ because }xy\cdot b_{1}\gtrdot y^{2}\cdot b_{1}
\]
\[
x^{2}\cdot b_{1}+100y^{2}\cdot b_{1}\gtrdot x^{2}\cdot b_{1},\mbox{ because the right ordered form is beginning of the left}
\]
\[
x^{2}\cdot b_{1}\gtrdot xy\cdot b_{1}+y^{2}\cdot b_{1}+x^{2}\cdot b_{2},\mbox{ because }x^{2}\cdot b_{1}\gtrdot xy\cdot b_{1}
\]
\[
xy\cdot b_{1}+y^{2}\cdot b_{1}+x^{2}\cdot b_{2}\gtrdot y\cdot b_{3}+y^{2}\cdot b_{1}+x^{2}\cdot b_{2},\mbox{ because }xy\cdot b_{1}\gtrdot y\cdot b_{3}
\]
\[
y\cdot b_{3}+y^{2}\cdot b_{1}+x^{2}\cdot b_{2}\mbox{ and }2y\cdot b_{3}+y^{2}\cdot b_{2}
\]
are not comparable because the greatest different elements are $y\cdot b_{3}$
and $2y\cdot b_{3}$.

The ordering is compatible with signature-safeness:
\begin{thm}
If two representations of $p$ has a relation $\sum_{k}m_{i_{k}}\cdot b_{i_{k}}\lessdot\sum_{k}m_{j_{k}}\cdot b_{j_{k}}$
and the second one is signature-safe representation then the first
one is signature-safe too.
\end{thm}
...
it is equal to $|G_{g}|$. So the existence of infinite $\gtrdot$-descending
sequence of representation elements would lead to existence of infinite
$\succ$-descending sequence of signatures. Combining this with well-foundness
of the relation $\prec$ on signatures we get the proof for well-foundness
of $\lessdot$ ordering on elements.

The straightforward proof for well-foundness of $\lessdot$ ordering
on representations following from well-foundness of $\lessdot$ on
elements is not very complex but to skip its strict details the theorem
2.5.5 of \cite{Multiset-Order} will be used. It states well-foundness
of an well-founded ordering lexicographically extended to finite multiset.
This applies to the representations because they form a subset in
the finite multiset of representation elements. So, we have
\begin{thm}
The $\lessdot$ ordering on representations is well-founded.
\end{thm}
...
representation of $mh$ contains an element $m_{k_{0}}\cdot b_{i_{k_{0}}}$
...
\begin{itemize}
\item a some element $m_{k'_{0}}\cdot b_{i_{k'_{0}}}$ in $mh$ representation
is selected. Note that $k'_{0}$ in some cases is not equal to $k_{0}$
\item some representation $m_{k'_{0}}b_{i_{k'_{0}}}=\sum_{l}m_{l}\cdot b_{i_{l}}$
is constructed for this element.
\item it is shown that constructed representation is $\lessdot$-smaller
than representation $m_{k'_{0}}b_{i_{k'_{0}}}=m_{k'_{0}}\cdot b_{i_{k'_{0}}}$ 
\end{itemize}
Now $m_{k'_{0}}\cdot b_{i_{k'_{0}}}$ in $mh=\sum_{k}m_{k}\cdot b_{i_{k}}$
is replaced by $\sum_{l}m_{l}\cdot b_{i_{l}}$ and after combining
coefficients near elements with both monomial and polynomial equal
a modified representation for $mh$ appears. Is is $\lessdot$-smaller
than $mh=\sum_{k}m_{k}\cdot b_{i_{k}}$ because all elements $\gtrdot$-greater
than $m_{k_{0}}\cdot b_{i_{k_{0}}}$ are identical in both representations
if they present. The element  $m_{k'_{0}}\cdot b_{i_{k'_{0}}}$ is
contained in original representation but not in the modified. And
all other elements in representations are $\lessdot$-smaller than
$m_{k'_{0}}\cdot b_{i_{k'_{0}}}$ so they doesn't influence the comparison.
The comparison holds even in a corner case when all elements are discarded
while combining coefficients. This case can appear if the original
representation is equal to $mh=m_{k'_{0}}\cdot b_{i_{k'_{0}}}+\sum_{l}(-m_{l})\cdot b_{i_{l}}$
what leads to modified representation $mh=0$ with zero elements which
is $\lessdot$-smaller than any non-empty representation. This result
can be expressed in the following 
\begin{lem}
If an element $m_{k'_{0}}\cdot b_{i_{k'_{0}}}$ of signature-safe
representation $mh=\sum_{k}m_{k}\cdot b_{i_{k}}$ has an representation
$m_{k'_{0}}b_{i_{k'_{0}}}=\sum_{l}m_{l}\cdot b_{i_{l}}$ which is
$\lessdot$-smaller than representation $m_{k'_{0}}b_{i_{k'_{0}}}=m_{k'_{0}}\cdot b_{i_{k'_{0}}}$
then $mh$ has a signature-safe representation $\lessdot$-smaller
than $mh=\sum_{k}m_{k}\cdot b_{i_{k}}$.
\end{lem}
...
three properties. An element not having the first property does satisfy
the F5 criterion and the idea is to use that $m_{k_{0}}\Sig(b_{i_{k_{0}}})$
is not the minimal signature of $m_{k_{0}}b_{i_{k_{0}}}$ like in
Theorem 20 of \cite{F5C}. $k'_{0}=k_{0}$ is taken for this case.
Consider input-representation of $m_{k_{0}}b_{i_{k_{0}}}$ with signature
of $\gtrdot$-maximal element equal to $m_{k_{0}}\Sig(b_{i_{k_{0}}})=s_{0}\mathbf{F}_{j_{0}}$:

\begin{equation}
m_{k_{0}}b_{i_{k_{0}}}=c_{0}s_{0}\cdot f_{j_{0}}+\sum_{l}m_{l}\cdot f_{i_{l}}.\label{eq:input-repr-case1}
\end{equation}
...
From this we can write another representation for $m_{k_{0}}b_{i_{k_{0}}}$,
...
\[
m_{k_{0}}b_{i_{k_{0}}}=\sum_{i=1}^{N_{0}}c_{0}s_{1}m_{0i}\cdot f_{j_{1}}+\sum_{i=2}^{N_{1}}-c_{0}s_{1}m_{1i}\cdot f_{j_{0}}+\sum_{l}m_{l}\cdot f_{i_{l}}.
\]
This representation is $\lessdot$-smaller than $m_{k_{0}}\cdot b_{i_{k_{0}}}$
...
the signatures are:$s_{1}m_{1i}\mathbf{F}_{j_{0}}\prec s_{1}m_{11}\mathbf{F}_{j_{0}}=s_{0}\mathbf{F}_{j_{0}}$.
This gives 
\begin{lem}
If a signature-safe $G_{g}$-representation $mh=\sum_{k}m_{k}\cdot b_{i_{k}}$
does not satisfy property 1 then there exists an element $m_{k'_{0}}\cdot b_{i_{k'_{0}}}$
having $G_{g}$-representation $m_{k'_{0}}b_{i_{k'_{0}}}=\sum_{l}m_{l}\cdot b_{i_{l}}$
which is $\lessdot$-smaller than representation $m_{k'_{0}}b_{i_{k'_{0}}}=m_{k'_{0}}\cdot b_{i_{k'_{0}}}$
.
\end{lem}
For the elements not satisfying case 2 the $\lessdot$-smaller representation
is created in a way used in Proposition 17 of \cite{F5C}. $k'_{0}=k_{0}$
is taken for this case too. Assume that $\Sig(m_{k_{0}}b_{i_{k_{0}}})=s_{0}\mathbf{F}_{j_{0}}$
and it is rewritten by labeled polynomial $b_{i'}$ from $R$. Because
the representation is signature-safe we have $\Sig(b_{i'})\preccurlyeq s_{0}\mathbf{F}_{j_{0}}\preccurlyeq\Sig(mh)\prec\Sig(g)$.
...
criterion definition we know that $i'>i_{k_{0}}$ and the existence
of $s'\in T$ such that $s'\Sig(b_{i'})=s_{0}\mathbf{F}_{j_{0}}$.
So, for the $m_{k_{0}}b_{i_{k_{0}}}$ there is an input-representation
\ref{eq:input-repr-case1} and for the $s'b_{i'}$ the input-representation
is:
...
\[
m_{k_{0}}b_{i_{k_{0}}}=c'^{-1}c_{0}s'\cdot b_{i'}+\sum_{l'}-c'^{-1}c_{0}m_{l'}\cdot f_{i_{l'}}+\sum_{l}m_{l}\cdot f_{i_{l}}
\]
It is $\lessdot$-smaller than $m_{k_{0}}b_{i_{k_{0}}}=m_{k_{0}}\cdot b_{i_{k_{0}}}$
because elements of both sums has signatures smaller than $s_{0}\mathbf{F}_{j_{0}}$,
and for the first element $ $$\Sig(c'^{-1}c_{0}s'\cdot b_{i'})=\Sig(m_{k_{0}}\cdot b_{i_{k_{0}}})=s_{0}\mathbf{F}_{j_{0}}$
but $i'>i_{k_{0}}$, so applying the $\lessdot$-comparison rule for
equal signatures and different list positions we get that element
$c'^{-1}c_{0}s'\cdot b_{i'}$ is $\lessdot$-smaller than $m_{k_{0}}\cdot b_{i_{k_{0}}}$
too. This gives
\begin{lem}
If a signature-safe $G_{g}$-representation $mh=\sum_{k}m_{k}\cdot b_{i_{k}}$
with $\Sig(mh)\prec\Sig(g)$ does not satisfy property 2 then there
exists an element $m_{k'_{0}}\cdot b_{i_{k'_{0}}}$ having $G_{g}$-representation
$m_{k'_{0}}b_{i_{k'_{0}}}=\sum_{l}m_{l}\cdot b_{i_{l}}$ which is
$\lessdot$-smaller than representation $m_{k'_{0}}b_{i_{k'_{0}}}=m_{k'_{0}}\cdot b_{i_{k'_{0}}}$
\textup{.}
\end{lem}
Now assume, that the all representation elements satisfy properties
1 and 2, but there exists at least one element $m_{k_{0}}\cdot b_{i_{k_{0}}}$
that does not satisfy property 3. Let $m_{\max}$ be the maximal $\HM$
of labeled polynomials corresponding to representation elements and
$H_{\max}$ be a list of elements where $m_{\max}$ is achieved. Select
$k'_{0}$ to be the index of the $\gtrdot$-greatest representation
element in $H_{\max}$. We have $ $$\HM(m_{k'_{0}}b_{i_{k'_{0}}})=m_{\max}\geqslant\HM(m_{k{}_{0}}b_{i_{k{}_{0}}})>\HM(mh)$,
so the HM of sum of all elements except $k'_{0}$ is equal to $\HM(mh-m_{k'_{0}}b_{i_{k'_{0}}})=\HM(m_{k'_{0}}b_{i_{k'_{0}}})=m_{\max}$,
so there is another element $k'_{1}$ having $\HM(m_{k'_{1}}b_{i_{k'_{1}}})=m_{\max}$.
So, $m_{k'_{1}}\cdot b_{i_{k'_{1}}}\in H_{\max}$ and $m_{k'_{1}}\cdot b_{i_{k'_{1}}}\lessdot m_{k'_{0}}\cdot b_{i_{k'_{0}}}$
because of $ $$m_{k'_{0}}\cdot b_{i_{k'_{0}}}$ $\gtrdot$-maximality
in $H_{\max}$.

The $\HM(m_{k'_{1}}b_{i_{k'_{1}}})=\HM(m_{k'_{0}}b_{i_{k'_{0}}})$
means that a critical pair of $b_{i_{k'_{0}}}$ and $b_{i_{k'_{1}}}$
has the form $[m'^{-1}m_{\max},\, m'^{-1}m_{k'_{0}},\, b_{i_{k'_{0}}},\, m'^{-1}m_{k'_{1}},\, b_{i_{k'_{1}}}]$
where $m'=\mbox{gcd}(m_{k'_{0}},m_{k'_{1}})$. Let $q$ be corresponding
S-polynomial. Then $m'\Sig(q)\preccurlyeq\Sig(mh)\prec\Sig(g)$ because
$m'\Sig(q)=\Sig(m_{k'_{0}}b_{i_{k'_{0}}})$ and the representation
is signature-safe. The S-polynomial parts $m'^{-1}m_{k'_{0}}b_{i_{k'_{0}}}$
and $m'^{-1}m_{k'_{1}}b_{i_{k'_{1}}}$ doesn't satisfy F5 and Rewritten
criteria because their forms multiplied by $m'$ are $m_{k'_{0}}b_{i_{k'_{0}}}$
and $m_{k'_{1}}b_{i_{k'_{1}}}$ -- labeled polynomials corresponding
to elements which are known not to satisfy both criteria by assumption.
Therefore $m'\Sig(q)\prec\Sig(g)$ and $\Sig(q)\prec\Sig(g)$. It
follows from this with theorem \ref{thm:Exist-gg-repr} that the S-pair
$(b_{i_{k'_{0}}},b_{i_{k'_{1}}})$ is S-pair with computed $G_{g}$-representation,
what means that there is an representation described in example \ref{example-of-having-gg-repr}
:
...
$\forall l\,\Sig(q)\succ\Sig(m_{l}b_{i_{l}})$ and $i'>k'_{0}$.

From the other hand we have $m'q=c_{0}m_{k'_{0}}b_{i_{k'_{0}}}-c_{1}m_{k'_{1}}b_{i_{k'_{1}}}$,
so we get the following representation:
\[
m_{k'_{0}}b_{i_{k'_{0}}}=c_{0}^{-1}c_{1}m_{k'_{1}}\cdot b_{i_{k'_{1}}}+c_{0}^{-1}m'\cdot b_{i'}+\sum_{l}c_{0}^{-1}m'm_{l}\cdot b_{i_{l}}.
\]
It is $\lessdot$-smaller than $m_{k'_{0}}b_{i_{k'_{0}}}=m_{k'_{0}}\cdot b_{i_{k'_{0}}}$: 
\begin{itemize}
\item $m_{k'_{1}}\cdot b_{i_{k'_{1}}}$ was already compared to $m_{k'_{0}}\cdot b_{i_{k'_{0}}}$ 
\item $m'\cdot b_{i'}$ has the same signature but greater position $i'>i_{k'_{0}}$
\item the last sum contains elements with signatures smaller than $m'\Sig(b_{i'})=\Sig(m_{k'_{0}}\cdot b_{i_{k'_{0}}})$.
\end{itemize}
This gives 
\begin{lem}
...
not satisfy property 3 then there exists an element $m_{k'_{0}}\cdot b_{i_{k'_{0}}}$
having representation $m_{k'_{0}}b_{i_{k'_{0}}}=\sum_{l}m_{l}\cdot b_{i_{l}}$
which is $\lessdot$-smaller than representation $m_{k'_{0}}b_{i_{k'_{0}}}=m_{k'_{0}}\cdot b_{i_{k'_{0}}}$
.
\end{lem}
Four previous lemmas together produce a theorem about existence $\lessdot$-smaller
representation:
\begin{thm}
A signature-safe representation $mh=\sum_{k}m_{k}\cdot b_{i_{k}}$
with $\Sig(mh)\prec\Sig(g)$ either satisfies properties 1-3 or there
exists a signature-safe representation $mh=\sum_{l}m_{l}\cdot b_{i_{l}}$
which is $\lessdot$-smaller than \textup{$\sum_{k}m_{k}\cdot b_{i_{k}}$.}
\end{thm}
In conjunction with $\lessdot$-order well-foundness this means that
from any given signature-safe representation we can produce a finite
$\lessdot$-descending sequence which ends with an signature-safe
representation satisfying properties 1-3. This leads to main result:
\begin{thm}
...
$mh=\sum_{k}m_{k}\cdot b_{i_{k}}$ that satisfies properties 1-3\textup{.}
\end{thm}
This result may be interesting by itself, but for the purposes of
proving termination only one corollary is needed:
\begin{cor}
\label{cor:all-needed-for-terminaton}If there exists a signature-safe
reductor $f$ for the labeled polynomial $f_{1}$ with $\Sig(f)\frac{\HM(f_{1})}{\HM(f)}\prec\Sig(g)$
then $G_{g}$ contains a signature-safe reductor for $f_{1}$ that
is not rejected by F5 and Rewritten criteria.
\end{cor}
To prove the corollary let $mf,\, m=\frac{\HM(f_{1})}{\HM(f)}\in\mathcal{K}\times T,\, f\in G_{g}$
...
equal to $\HM(mf)$ there exists an element $k_{0}$ that achieves
HM equality: $\HM(m_{k_{0}}\cdot b_{i_{k_{0}}})=\HM(mf)=\HM(f_{1})$.
Since the representation is signature-safe $\Sig(m_{k_{0}}\cdot b_{i_{k_{0}}})\preccurlyeq\Sig(mf)\prec\Sig(f_{1})$
so $m_{k_{0}}b_{i_{k_{0}}}$ is a signature-reductor for $f_{1}$
and properties 1-2 ensure that $m_{k_{0}}b_{i_{k_{0}}}$ does not
satisfy criteria. The corollary is proved.
...
for the case of algorithm non-termination existence of a polynomial
$f$ such that $\HM(f)|\HM(g),$ $\Sig(f)|\Sig(g),$ $\frac{\HM(f)}{\Sig(f)}>\frac{\HM(g)}{\Sig(g)},$
where $g$ is the same fixed $g$ as above. Let $t=\frac{\HM(g)}{\HM(f)}$.
$tf$ is a signature-safe reductor for $g$, so $\Sig(f)t\prec\Sig(g)$
and the corollary \ref{cor:all-needed-for-terminaton} can be applied
to find a signature-safe reductor $t_{1}f_{1}$ for $g$ which does
not satisfy criteria. Also the reductor $f_{1}$ is known to belong
to $G_{g}$, so during the algorithm execution it was appended to
$G\cup Done$ before $g$. This gives main result for establishing
contradiction:
\begin{thm}
If the algorithm does not terminate for some input then after some
finite step the set \textup{$G\cup Done$} contains a pair of labeled
polynomials $f_{1},g$ where:

-$f_{1}$ is added to \textup{$G\cup Done$} before $g$

- $t_{1}f_{1}$ does not satisfy F5 and Rewritten criteria, where
$t_{1}=\frac{\HM(g)}{\HM(f_{1})}$

-$f_{1}$ is signature-safe reductor for $g$.
\end{thm}
From the other hand existence of such pair of labeled polynomials
is not possible: consider the call to TopReduction after which the
polynomial $g$ was inserted in $Done$. That call returns polynomial
as first part of TopReduction return value, so the value returned
by IsReducible is empty set. It means that one of conditions (a)-(d)
was not satisfied for all polynomials in $G\cup Done$ including $f$.
This is not possible because:
\begin{itemize}
\item (a) is satisfied because $f_{1}$ is a reductor for $g$ from last
theorem
\item (b) and (c) are satisfied because $\frac{\HM(g)}{\HM(f_{1})}f_{1}$
does not satisfy F5 and Rewritten criteria from last theorem
\item (d) is satisfied because $f_{1}$ is a signature-safe reductor for
$g$ from last theorem.
\end{itemize}
So, the contradiction is proved and the main result too:
\begin{thm}
...
is based on well-foundness and doesn't give any such limit too. Unfortunately
the termination proof given here is quite different in structure compared
to the proof of Buchberger algorithm termination, so this proof doesn't
show that F$_{5}$ is more efficient than Buchberger in any sense.
Unlike this the termination of the modified versions of F$_{5}$ algorithm
in \cite{term-mod-1,term-mod-2,Modifying-for-termination} is shown
in a way analogous to Buchberger algorithm and there is room for comparison
of their efficiency with Buchberger's one.
...
for calculating Gröbner bases for polynomial ideals

\bibitem{F5C}Christian Eder, John Perry. F5C: a variant of Faugère's
F5 algorithm with reduced Gröbner bases

\bibitem{Modifying-for-termination}Christian Eder, Justin Gash, and
John Perry. Modifying Faugère\textquoteright{}s F5 algorithm to ensure
termination

\bibitem{Multiset-Order}Franz Baader and Tobias Nipkow. Term Rewriting
and All That

\bibitem{key-1}Gwénolé Ars, Amir Hashemi. Extended F 5 criteria.

\bibitem{term-mod-1}Gwénolé Ars. Applications des bases de Gröbner
à la cryptographie

\bibitem{F5-Orig}Jean-Charles Faugère. A new effi{}cient algorithm
for computing Gröbner bases without reduction to zero (F$_{5}$)

\bibitem{term-mod-2}Justin M. Gash. On efficient computation of Gröbner
bases

\bibitem{F5-Revisited}Till Stagers. Faugère\textquoteright{}s F5
