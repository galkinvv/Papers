...
$r_{L}$ exist. This finishes the theorem proof. The set of ends of
those 2-length S-pair-chains will be called \emph{S-pair-descendants}
of $r_{L}$.
...
\end{itemize}
Now $m_{K'}\cdot b_{i_{K'}}$ in $mh=\sum_{k}m_{k}\cdot b_{i_{k}}$
...
than $m_{K'}\cdot b_{i_{K'}}$ are identical in both representations
if they present. The element  $m_{K'}\cdot b_{i_{K'}}$ is contained
in original representation but not in the modified. And all other
elements in representations are $\lessdot$-smaller than $m_{K'}\cdot b_{i_{K'}}$
so they doesn't influence the comparison. The comparison holds even
in a corner case when all elements are discarded while combining coefficients.
...
the F5 criterion and the idea is to use that $m_{K}\Sig(b_{i_{K}})$
is not the minimal signature of $m_{K}b_{i_{K}}$ like in Theorem
20 of \cite{F5C}. $K'=K$ is taken for this case. Consider input-representation
of $m_{K}b_{i_{K}}$ with signature of $\gtrdot$-maximal element
equal to $m_{K}\Sig(b_{i_{K}})=s_{0}\mathbf{F}_{j_{0}}$:
...
is taken for this case too. Assume that $\Sig(m_{K}b_{i_{K}})=s_{0}\mathbf{F}_{j_{0}}$
...
but $i'>i_{K}$, so applying the $\lessdot$-comparison rule for equal
signatures and different list positions we get that element $c'^{-1}c_{0}s'\cdot b_{i'}$
is $\lessdot$-smaller than $m_{K}\cdot b_{i_{K}}$ too. This gives
\begin{lem}
If a signature-safe $G_{g}$-representation $mh=\sum_{k}m_{k}\cdot b_{i_{k}}$
...
1 and 2, but there exists at least one element $m_{K}\cdot b_{i_{K}}$
...
$K'$ to be the index of the $\gtrdot$-greatest representation element
in $H_{\max}$. We have $ $$\HM(m_{K'}b_{i_{K'}})=m_{\max}\geqslant\HM(m_{k{}_{0}}b_{i_{k{}_{0}}})>\HM(mh)$,
...
So, $m_{K''}\cdot b_{i_{K''}}\in H_{\max}$ and $m_{K''}\cdot b_{i_{K''}}\lessdot m_{K'}\cdot b_{i_{K'}}$
because of $ $$m_{K'}\cdot b_{i_{K'}}$ $\gtrdot$-maximality in
$H_{\max}$.
...
It is $\lessdot$-smaller than $m_{K'}b_{i_{K'}}=m_{K'}\cdot b_{i_{K'}}$: 
\begin{itemize}
\item $m_{K''}\cdot b_{i_{K''}}$ was already compared to $m_{K'}\cdot b_{i_{K'}}$ 
\item $m'\cdot b_{i'}$ has the same signature but greater position $i'>i_{K'}$
\item the last sum contains elements with signatures smaller than $m'\Sig(b_{i'})=\Sig(m_{K'}\cdot b_{i_{K'}})$.
\end{itemize}
This gives 
\begin{lem}
...
equality: $\HM(m_{K}\cdot b_{i_{K}})=\HM(mf)=\HM(f_{1})$. Since the
representation is signature-safe $\Sig(m_{K}\cdot b_{i_{K}})\preccurlyeq\Sig(mf)\prec\Sig(f_{1})$
so $m_{K}b_{i_{K}}$ is a signature-reductor for $f_{1}$ and properties
1-2 ensure that $m_{K}b_{i_{K}}$ does not satisfy criteria. The corollary
is proved.
...
-$t_{1}f_{1}$ does not satisfy F5 and Rewritten criteria, where $t_{1}=\frac{\HM(g)}{\HM(f_{1})}$
