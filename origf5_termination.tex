...
by transitively extending the monomial ordering: $\frac{m_{1}}{m_{2}}>\frac{m_{3}}{m_{4}}\Leftrightarrow m_{1}m_{4}>m_{3}m_{2}$.
\begin{thm}
...
\Sig(f)|\Sig(g),
\]
where \textgreater{} is quotient ordering.\end{thm}
\begin{proof}
...
ordering -- for example the sequence $\frac{x}{x}>\frac{x}{x^{2}}>\cdots>\frac{x}{x^{n}}>\cdots$
...
while $i_{1}\ne i_{2}$. Also theorem 1 of \cite{F5-Orig} states
that all polynomials in the algorithm are admissible, do the above
claim will can be applied to all appeared polynomials.
\begin{example}
...
following from $\lessdot$-well-orderness of elements is not very
...
because all elements $\gtrdot$-greater than $m_{K'}\cdot b_{i_{K'}}$
...
are $\lessdot$-smaller than $m_{K'}\cdot b_{i_{K'}}$ so they doesn't
influence the comparison. The comparison holds even in a corner case
when all elements are discarded while combining coefficients. This
case can appear if the original representation is equal to $mh=m_{K'}\cdot b_{i_{K'}}+\sum_{l}(-m_{l})\cdot b_{i_{l}}$
...
of $\gtrdot$-maximal element equal to $m_{K}\Sig(b_{i_{K}})=s_{0}\mathbf{F}_{j_{0}}$:
...
is $\lessdot$-smaller than $m_{K}\cdot b_{i_{K}}$ too.\end{proof}
\begin{lem}
...
to be the index of the $\gtrdot$-greatest representation element
...
is not rejected by F5 and Rewritten criteria.\end{cor}
\begin{proof}
Let $mf,\, m=\frac{\HM(f_{1})}{\HM(f)}\in\mathcal{K}\times T,\, f\in G_{g}$
...
where $g$ is the same fixed $g$ as above. Using this result and
last corollary we construct two polynomials leading to contradiction
...
contains a pair of labeled polynomials $f_{1},g$ where:
\begin{itemize}
\item $f_{1}$ is added to \textup{$G\cup Done$} before $g$
\item $t_{1}f_{1}$ does not satisfy F5 and Rewritten criteria, where $t_{1}=\frac{\HM(g)}{\HM(f_{1})}$
\item $f_{1}$ is signature-safe reductor for $g$.
\end{itemize}
\end{thm}
\begin{proof}
Let $g,f$ be polynomials from the theorem \ref{thm:f_g_3_props}
an define $t=\frac{\HM(g)}{\HM(f)}$. $tf$ is a signature-safe reductor
for $g$, so $\Sig(f)t\prec\Sig(g)$ and the corollary \ref{cor:all-needed-for-terminaton}
can be applied to find a signature-safe reductor $t_{1}f_{1}$ for
$g$ which does not satisfy criteria. Also the reductor $f_{1}$ is
known to belong to $G_{g}$, so during the algorithm execution it
was appended to $G\cup Done$ before $g$.\end{proof}
\begin{thm}
The original $ $F$_{5}$ algorithm as described in \cite{F5-Orig}
does terminate for any input.\end{thm}
\begin{proof}
We are going o show that the existence of polynomials $f_{1},g$ from
the theorem \ref{thm:always-exist-ok-reductor} leads to contradiction.
Consider the call to TopReduction after which the polynomial $g$
was inserted in $Done$. That call returns polynomial $g$ as first
...
for all polynomials in $G\cup Done$ including $f_{1}$. This is not
possible because:
\begin{itemize}
\item (a) is satisfied because $f_{1}$ is a reductor for $g$ from the
...
$g$ from the theorem \ref{thm:always-exist-ok-reductor}.
\end{itemize}
\end{proof}
