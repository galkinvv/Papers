...
хотя бы одно из двух неравенств: $h_{1}\GVWl h_{3}$ или $h_{3}\GVWl h_{2}$.
\end{lem}
...
\[
\exists K\in k\exists v\in P:\HM(v)=\sigma'\prec_{0}\sigma,vf\equiv\poly(h_{1})-K\poly(h_{2})\pmod{I_{0}},
\]
...
основанных на сигнатурах -- алгоритмом G2V из работы {[}2{]}, версией
F5, опубликованную в работе {[}1{]} и схеме алгоритма TRB-MJ, предложенной
Lei Huang в статье \char`\"{}A new conception for computing Gr\"{o}bner
basis and its applications\char`\"{}, депонированной на \href{http://arxiv.org/abs/1012.5425v2}{http://arxiv.org/abs/1012.5425v2}.
Все они в определённом смысле являются модификациями простого F5,
впервые представленного в работе {[}3{]}, причём модификации направлены
на сокращение числа редукций многочленов, занимающих большую часть
времени в процессе вычисления базисов Грёбнера. Первый из них отличается
от немодифицированной версии тем, что не использует явного вычисления
S-полиномов до проведения редукции и вводит специальный критерий \char`\"{}super-topreducible\char`\"{}
для отбрасывания некоторых многочленов после их редукции. Второй применяет
расширенный критерий, отбрасывая до редукции те из посчитанных S-полиномов,
которые не смогут удовлетворять определённому в работе критерию \char`\"{}primitve
S-irreducble\char`\"{} после редукции. Эти методики позволяют ускорить
работу алгоритма, однако в обоих случаях в определённый момент отбрасывается
многочлен, полученный в процессе предыдущих вычислений путём редукции
или создания S-полинома.

В алгоритме SimpleSignatureGroebner S-полиномы не вычисляются явно
и их отбрасывание осуществляется на основе критерия, записываемого
в точности как критерий алгоритма из {[}1{]}. При этом для проверки
на шаге \ref{enu:-remove-from-B} элемента $B$ достаточно знать лишь
его сигнатуру и старший моном. Поскольку каждый элемент $B$ получается
как домноженный на моном $m$ элемент $r\in R$ их можно хранить в
виде пары $\left(m,r\right)$, не выполняя без необходимости операции
умножения всего многочлена на моном. Таким образом, для отброшенных
элементов $B$ не производится никаких операций, сложность которых
пропорциональна длине многочлена. Заметим, что применяемое в алгоритме
из {[}1{]} вычисление старшего монома S-пары в общем случае напротив
имеет именно такую сложность. Это делает возможной реализацию алгоритма
SimpleSignatureGroebner, в которой все длительные операции над многочленами
производятся лишь для многочленов, попадающих в результирующее множество.
Хотя результирующее множество не является минимальным базисом Грёбнера,
оно является минимальным S-базисом идеала, дополненным сигнатурами
некоторых нулевых многочленов. Отсюда можно сделать вывод о некоторой
оптимальности критериев отбрасывания в случае регулярной входной последовательности:
...
и не лишив результат свойства быть S-базисом.

Другим отличием алгоритма SimpleSignatureGroebner от двух рассмотренных
выше и от TRB-MJ является отсутствие явной проверки критериев, связанных
с \emph{сизигиями} -- нулевыми многочленами ненулевой сигнатуры. Это
вызвано тем, что сизигии являются $\GVWl$-меньшими элементами $R$
и участвуют в отбрасывании элементов $B$ наравне с другими многочленами,
реализуя критерии отбрасывания эквивалентные алгоритму G2V, являющиеся
оптимальными в смысле содержания минимального базиса идеала сигнатур
сизигий в моноиде $\mathbb{T}$.

Вопрос наиболее эффективного способа выбора редуктора в \nameref{par:ReduceCheckingSignatures:}
является открытым. Представленный в этой работе способ выбора, основанный
на $\GVWl$-сравнении совпадает со способом выбора, применявшегося
в оригинальном алгоритме F5 для случая однородных многочленов позволяет
осуществить этот выбор на основе всё того же упорядочения множества
$R$.

Таким образом алгоритм SimpleSignatureGroebner реализует все критерии
отбрасывания и выбора редуктора на основе одного и того же простого
порядка $\GVWl$ на $R$, что и позволяет сформулировать и создать
его простую реализацию без дополнительных списков многочленов (как
в G2V и TRB-MJ) или правил (как в F5), которая оказывается не менее
эффективной чем указанные алгоритмы.
...
из авторов {[}4{]}) по реализации F5-подобных алгоритмов на этом ядре.
Исходный код реализации содержится в функции ssg файла, доступного
по адресу \href{https://github.com/galkinvv/Singular-f5-like/blob/ssg/kernel/kstd2.cc}{https://github.com/galkinvv/Singular-f5-like/blob/ssg/kernel/kstd2.cc}

Сравнение реализации SimpleSignatureGroebner с другими алгоритмами
вычисления базисов Грёбнера, реализованных Christian Eder подтвердили
...
\wref{2}{Gao S., Guan Y., Volny F.} A new incremental algorithm for computing Groebner bases // Proceedings of the 2010 International Symposium on Symbolic and Algebraic Computation. ACM. New York. 2010. 13--19.

\wref{3}{Faug\`{e}re J.-C.} {A new efficient algorithm for computing Gr\"{o}bner bases without reduction to zero (F5) // Proceedings of the 2002 International Symposium on Symbolic and Algebraic Computation. ACM. New York. 2002. 75--83.

\wref{4}{Eder C., Perry J.} {Signature-based algorithms to compute Gr\"{o}bner bases // Proceedings of the 36th International Symposium on Symbolic and Algebraic Computation. ACM. New York. 2011. 99--106. }
\lend
\end{document}
