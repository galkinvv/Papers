...
the step\ref{enu:-before-reduce} of any algorthm iteration -- the
...
\ref{enu:-after-reduce} of any algorthm iteration -- the highest
...
the step \ref{enu:-after-reduce} of any algorthm iteration -- no
one from $R$ elements has simultaneousely a highest monomial dividing
...
with non-zero $p$ during every iteraton we have $\HM(p)$ decrease
according to $\prec_{0}$, which is possible only finite number of
times. When $p$ becomes zero it stops immediately because of $\GVWl$-minmality
of $(\sigma,0)$.

The set $R\subset\mathbb{T}_{0}\times P$ is extended n every step
of the main algorthm cycle. It can be splitted to $R_{*0}\cup R_{0*}\cup R_{**},$
...
and $R_{**}$ we apply a method based on idea of monoid ideals introduced
in \cite{KreuzerRobbianoBook1} as ``monideal''. Consider the following
...
can be applied to thier monoideals. It states exactly the needed fact
...
-- S-representation of $h$. Then at least one $j$ satisfies $\HM(h)=\HM(t_{j}r_{i_{j}})$.
\end{lem}
...
\begin{defn}
The labeled polynomial set $R\subset H$\emph{ is called S-basis}
(correspondingly \emph{S$_{\sigma}$-basis}), if all elements of $H$
(correspondingly $\left\{ h\in H\,|\,\SIG(h)\prec_{0}\sigma\right\} $)\emph{
}have S-representation over $R$\emph{.}\end{defn}
\begin{lem}
\label{lem:s-basis-and-no-reductions}Let $\sigma\succ_{0}0,R=\left\{ r_{i}\right\} $
-- S\emph{$_{\sigma}$}-basis and $h_{1},h_{2}\in H,\SIG(h_{i})=\sigma$
are labeled polynomials, that can't be signature-safe reduced by $R$
...
having signature $\prec_{0}\sigma$. Ths can be wrtten as: 
\[
...
\ref{enu:-remove-from-B} the followong invariant holds: for $\forall\sigma\in\mathbb{T},\sigma\prec$
...
first algorthm iteration with $\SIG(r_{0})=1$. Let $r_{\sigma}$
...
\ref{enu:-remove-from-B} the followong invariant holds: $\forall h\in H,\SIG(h)\prec$
...
Then $R$ -- S\emph{$_{\sigma}$}-basis. $\forall g\in I_{0}\,\left(0,g\right)$
...
can be replaced by $t_{\sigma}r_{\sigma}$ to acquire representation
of$v_{\sigma}$ over $R$ only. It's existance leads to contradiction.
\begin{lem}
...
\ref{thm:has-s-repr} $R$ -- S-basis.


\section{Comparison with other algorithms}

The presented algorthm belongs to the family of the Gröbner basis
algorithms using signatures and being at some degree a modification
of F5 algorthm from \cite{FaugereF5}. One of the main modification
directions of F5 is simplifying and clarfying the connected theory
...
criteria to detect and dont't perform some unnecessary computations.
It is studied in \cite{F5C,G2V,SignatureBasedGBs} and allows to perform
coputations in a way that reduces to the end only polynomials that
are either new S-basis entries or corresponds to new signature of
zero polynomial -- called \emph{syzygy signature} -- that extend the
monoideal of such signatures\emph{.} Generalisation simultaneousely
using all criteria idescribed in algorithms TRB-MJ and SB \cite{HuangConception,PracticalGB}
achieve even more efficiency because all discardings are performed
before any computational heavy operations like polynomial reduction
or computation highest monomial of S-polynomial -- so the non-trivial
computations are never become unnecessary because their results are
never discarded.
...
same orderng of $R$ and is identical for homogeneous case with the
methos used in original F5. The correctnes proof is given without
...
Algorithm simplification lead to decreasing the time needed for its
programming implementation and debugging. It is achieved by the smaller
number of variables and using the same order for discarding criteria
and reductor selection. Simplicity of implementation and the absence
of complex data structures allows quick algorithm integration with
any computer algebra system. The author's implementation linked below
was written from scratch in a 8 hours what is a lot smaller than the
time author spent implementing other algorithms with a similar tools.
The proof based on S-represenation invariants make algorithm flow
more clear from the algebraic point of view and allows for algorithm
extension to other algrebraic objects similar to polynomial ring over
field.

The algorithm was implemented in С++ using low-level functions from
...
functon in a file available at \href{https://github.com/galkinvv/Singular-f5-like/blob/ssg/kernel/kstd2.cc}{https://github.com/galkinvv/Singular-f5-like/blob/ssg/kernel/kstd2.cc}
