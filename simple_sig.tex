...
order$\prec$. В этом кольце может быть поставлена задача вычисления
базиса Грёбнера для произвольного идеала $\left(f_{1},\dots,f_{l}\right)$.
Один из способов её решения инкрементальный: последовательно вычисляются
базисы идеалов $\left(f_{1},\dots,f_{i}\right),i=2\dots l$ на основе
уже вычисленного для идеала $\left(f_{1},\dots,f_{i-1}\right)$ базиса
$R_{i-1}$ и многочлена $f_{i}$. Представляемый алгоритм позволяет
